...
def _scenario_passed(name: str, result: RunMetrics) -> bool:
    ...
    if name.startswith("primary_flaky"):
        # Half the primary calls fail; cache + fallback should keep us mostly up
        # and the breaker must engage. (Whether it also fully heals within the
        # run depends on how warm the cache is — see aggregate recovery_time_ms
        # and reports/scenarios.json for the open->closed evidence.)
        return result.availability >= 0.8 and result.circuit_open_count >= 1
...
def run_simulation(config: LabConfig, queries: list[str]) -> RunMetrics:
    ...
    Seeds the RNG first so the whole run (query choice + simulated provider
    failures) is reproducible.
    """
